...
import { CalendarIcon, Search, Filter, DollarSign, TrendingUp, TrendingDown } from "lucide-react";
...
export const BankTransactionsTable: React.FC<BankTransactionsTableProps> = ({
  ...
  const fetchTransactions = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "No se pudieron cargar las transacciones bancarias",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Movimientos Bancarios ({filteredTransactions.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {filteredTransactions.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-muted-foreground">No se encontraron transacciones bancarias</p>
            </div>
          ) : (
