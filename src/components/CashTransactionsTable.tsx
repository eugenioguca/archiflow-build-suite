...
export const CashTransactionsTable: React.FC<CashTransactionsTableProps> = ({
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const [cashAccounts, clients, projects, suppliers] = await Promise.all([
      ...
      const formattedTransactions: CashTransaction[] = (data || []).map((transaction: any) => {
        const cashAccount = cashAccounts.data?.find(ca => ca.id === transaction.account_id);
        ...
        return {
          ...
          cash_account_name: cashAccount?.name || 'N/A',
