-- Create the approved two-table model for executive budget (corrected references)
...
CREATE TABLE IF NOT EXISTS public.presupuesto_ejecutivo_partida (
  ...
  parametrico_id UUID NOT NULL REFERENCES public.presupuesto_parametrico(id) ON DELETE CASCADE,
  importe_ejecutivo NUMERIC(14,2) NOT NULL DEFAULT 0,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  created_by UUID NOT NULL,
  UNIQUE (cliente_id, proyecto_id, parametrico_id)
);
...
CREATE TABLE IF NOT EXISTS public.presupuesto_ejecutivo_subpartida (
  ...
  cliente_id UUID NOT NULL,
  proyecto_id UUID NOT NULL,
  partida_ejecutivo_id UUID NOT NULL REFERENCES public.presupuesto_ejecutivo_partida(id) ON DELETE CASCADE,
...
CREATE OR REPLACE FUNCTION public.recalc_pep_total(pep_id UUID)
RETURNS VOID AS $$
BEGIN
  UPDATE public.presupuesto_ejecutivo_partida
  SET 
