...
DO $$ BEGIN
  ...
  ALTER TABLE progress_photos ADD COLUMN IF NOT EXISTS taken_at timestamptz NOT NULL DEFAULT now();
  ...
  ALTER TABLE progress_photos ADD COLUMN IF NOT EXISTS visibility text DEFAULT 'internal' CHECK (visibility IN ('internal', 'client', 'public'));
EXCEPTION
...
-- Create triggers for updated_at timestamps (only if not exists)
DO $$ BEGIN
...
-- Insert default partidas catalog structure (only if empty)
INSERT INTO partidas_catalog (codigo, nombre, descripcion, unidad, categoria, nivel, created_by)
SELECT '01', 'PRELIMINARES', 'Trabajos preliminares y preparación del sitio', 'm²', 'otros', 1, (SELECT id FROM profiles WHERE role = 'admin' LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM partidas_catalog WHERE codigo = '01');
...
) AS t(codigo, nombre, descripcion, unidad, categoria, nivel),
(SELECT id FROM profiles WHERE role = 'admin' LIMIT 1) AS admin_id
WHERE NOT EXISTS (SELECT 1 FROM partidas_catalog WHERE partidas_catalog.codigo = t.codigo);
...
UPDATE partidas_catalog SET parent_id = (SELECT id FROM partidas_catalog WHERE codigo = '04') WHERE codigo LIKE '04.%' AND codigo != '04' AND parent_id IS NULL;
UPDATE partidas_catalog SET parent_id = (SELECT id FROM partidas_catalog WHERE codigo = '05') WHERE codigo LIKE '05.%' AND codigo != '05' AND parent_id IS NULL;
...
UPDATE partidas_catalog SET parent_id = (SELECT id FROM partidas_catalog WHERE codigo = '07') WHERE codigo LIKE '07.%' AND codigo != '07' AND parent_id IS NULL;
UPDATE partidas_catalog SET parent_id = (SELECT id FROM partidas_catalog WHERE codigo = '08') WHERE codigo LIKE '08.%' AND codigo != '08' AND parent_id IS NULL;