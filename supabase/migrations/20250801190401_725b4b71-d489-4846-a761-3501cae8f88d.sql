...
-- Separate policies for design phases
...
CREATE POLICY "Team members can insert project phases" ON public.design_phases FOR INSERT
WITH CHECK (project_id IN (SELECT project_id FROM project_team_members WHERE user_id = (SELECT id FROM profiles WHERE user_id = auth.uid())) 
       OR EXISTS (SELECT 1 FROM profiles p WHERE p.user_id = auth.uid() AND p.role IN ('admin', 'employee')));

CREATE POLICY "Team members can update project phases" ON public.design_phases FOR UPDATE
USING (project_id IN (SELECT project_id FROM project_team_members WHERE user_id = (SELECT id FROM profiles WHERE user_id = auth.uid())) 
       OR EXISTS (SELECT 1 FROM profiles p WHERE p.user_id = auth.uid() AND p.role IN ('admin', 'employee')));

CREATE POLICY "Team members can delete project phases" ON public.design_phases FOR DELETE
USING (project_id IN (SELECT project_id FROM project_team_members WHERE user_id = (SELECT id FROM profiles WHERE user_id = auth.uid())) 
       OR EXISTS (SELECT 1 FROM profiles p WHERE p.user_id = auth.uid() AND p.role IN ('admin', 'employee')));

-- Separate policies for design appointments
...
CREATE POLICY "Team members can insert design appointments" ON public.design_appointments FOR INSERT
WITH CHECK (project_id IN (SELECT project_id FROM project_team_members WHERE user_id = (SELECT id FROM profiles WHERE user_id = auth.uid())) 
       OR EXISTS (SELECT 1 FROM profiles p WHERE p.user_id = auth.uid() AND p.role IN ('admin', 'employee')));

CREATE POLICY "Team members can update design appointments" ON public.design_appointments FOR UPDATE
USING (project_id IN (SELECT project_id FROM project_team_members WHERE user_id = (SELECT id FROM profiles WHERE user_id = auth.uid())) 
       OR EXISTS (SELECT 1 FROM profiles p WHERE p.user_id = auth.uid() AND p.role IN ('admin', 'employee')));

CREATE POLICY "Team members can delete design appointments" ON public.design_appointments FOR DELETE
USING (project_id IN (SELECT project_id FROM project_team_members WHERE user_id = (SELECT id FROM profiles WHERE user_id = auth.uid())) 
       OR EXISTS (SELECT 1 FROM profiles p WHERE p.user_id = auth.uid() AND p.role IN ('admin', 'employee')));
