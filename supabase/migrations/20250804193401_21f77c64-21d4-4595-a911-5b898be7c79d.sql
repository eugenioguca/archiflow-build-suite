-- Create hierarchical permission system while preserving client-project architecture
...
-- Add department and position to profiles table
ALTER TABLE public.profiles 
ADD COLUMN department department_type,
ADD COLUMN position position_hierarchy,
...
SET position = 'direccion_general', department = 'finanzas' 
WHERE role = 'admin';
...
BEGIN
  -- Get user profile
  SELECT p.role, p.department, p.position INTO user_profile
  ...
  RETURN EXISTS (
    ...
    WHERE dp.department = user_profile.department
    AND dp.position = user_profile.position
...
BEGIN
  ...
  IF user_profile.role = 'admin' OR user_profile.position = 'director' THEN
  ...
  IF user_profile.department IN ('finanzas', 'contabilidad') THEN
    SELECT ARRAY(SELECT id FROM public.branch_offices WHERE active = true) INTO branch_ids;
    RETURN branch_ids;
  END IF;
