...
-- 3. Función para normalizar attendees (maneja tanto arrays UUID como JSONB)
...
DECLARE
  ...
  profile_email TEXT;
BEGIN
  -- Si attendees es un array de UUIDs, convertir a JSONB
  IF NEW.attendees IS NOT NULL THEN
    -- Manejar diferentes tipos de datos en attendees
    BEGIN
      -- Intentar tratar como JSONB array
    ...
    EXCEPTION 
      WHEN OTHERS THEN
        -- Si falla el JSONB, probablemente es un array de UUIDs, mantener como está
        NULL;
    END;
  END IF;
