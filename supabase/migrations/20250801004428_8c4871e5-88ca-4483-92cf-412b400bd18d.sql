...
-- Cash flow projections table (fixed - removed nested generated columns)
CREATE TABLE public.cash_flow_projections (
  ...
  projected_net_flow NUMERIC DEFAULT 0, -- calculated in application or trigger
  actual_income NUMERIC DEFAULT 0,
  actual_expenses NUMERIC DEFAULT 0,
  actual_net_flow NUMERIC DEFAULT 0, -- calculated in application or trigger
  variance NUMERIC DEFAULT 0, -- calculated in application or trigger
  notes TEXT,
  created_by UUID NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
...
-- Function to update cash flow projection calculated fields
...
-- Add trigger for cash flow calculations
CREATE TRIGGER cash_flow_calculations_trigger
BEFORE INSERT OR UPDATE ON public.cash_flow_projections
FOR EACH ROW
EXECUTE FUNCTION update_cash_flow_calculations();
