-- ELIMINAR TODOS LOS DATOS DEL MÓDULO DE FINANZAS (CORREGIDO)
...
-- Eliminar referencias de pagos de tesorería (si existen datos)
DELETE FROM public.treasury_payment_references;

-- Eliminar pagos de materiales de tesorería (si existen datos)
DELETE FROM public.treasury_material_payments;

-- Eliminar elementos de pagos de materiales (si existen datos)
DELETE FROM public.treasury_material_payment_items;

-- Eliminar solicitudes de finanzas de materiales (si existen datos)
...
-- Crear política para suppliers (solo si no existe)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'suppliers' AND policyname = 'Employees and admins can manage suppliers') THEN
...
-- Crear política para material_requirements (solo si no existe)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'material_requirements' AND policyname = 'Employees and admins can manage material requirements') THEN
