-- Step 1: Create missing employee_has_project_access function
...
AS $$
BEGIN
  -- Check if the employee profile has access to the project
  -- Employees with admin role have access to all projects
  -- Other employees only have access if they are assigned to the project
  RETURN EXISTS (
    ...
    AND (
      ...
      OR EXISTS (
        ...
        AND (
          cp.assigned_advisor_id = profile_id_param
          OR cp.project_manager_id = profile_id_param
          OR cp.construction_supervisor_id = profile_id_param
        )
      )
    )
  );
...
-- Step 3: Create/update function to check client project access using profiles.id
