-- Corregir la función delete_client_cascade eliminando referencias a tablas inexistentes
...
BEGIN
  ...
  -- Delete client project calendar event alerts first
  ...
  -- Delete client project calendar events
  DELETE FROM public.client_project_calendar_events 
  WHERE client_project_id IN (
    SELECT id FROM public.client_projects WHERE client_id = client_id_param
  );
  
  -- Delete chat notifications for projects of this client
  ...
  -- Delete budget change log for projects of this client
  ...
  -- Delete documents for projects of this client (SISTEMA UNIFICADO)
  DELETE FROM public.documents 
  WHERE project_id IN (
    SELECT id FROM public.client_projects WHERE client_id = client_id_param
  );
  
  -- Delete documents directly linked to client (SISTEMA UNIFICADO)
  DELETE FROM public.documents WHERE client_id = client_id_param;
  
  -- Delete expenses for this client
  DELETE FROM public.expenses WHERE client_id = client_id_param;
  
  -- Delete unified financial transactions for projects of this client (using correct column name)
  DELETE FROM public.unified_financial_transactions 
  WHERE empresa_proyecto_id IN (
    SELECT id FROM public.client_projects WHERE client_id = client_id_param
  );
  
  -- Delete treasury transactions for projects of this client
  ...
  -- Delete project chat messages for projects of this client
  DELETE FROM public.project_chat 
  WHERE project_id IN (
    SELECT id FROM public.client_projects WHERE client_id = client_id_param
  );
