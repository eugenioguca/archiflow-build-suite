-- =====================================================
-- DATOS DE PRUEBA BÁSICOS FINALES PARA MÓDULO DE FINANZAS
-- =====================================================
...
BEGIN
    ...
    -- Crear anticipos de empleados (sin especificar amount_pending que es generado)
    INSERT INTO public.employee_advances (employee_name, advance_amount, amount_justified, status, advance_date, due_date, purpose, notes, created_by)
    SELECT 'Carlos Martínez', 35000.00, 20000.00, 'pending', '2024-01-15', '2024-03-01', 'Gastos de obra', 'Pendiente justificación', admin_profile_id
    WHERE NOT EXISTS (SELECT 1 FROM public.employee_advances WHERE employee_name = 'Carlos Martínez');

    INSERT INTO public.employee_advances (employee_name, advance_amount, amount_justified, status, advance_date, due_date, purpose, notes, created_by)
    SELECT 'Ana López', 25000.00, 25000.00, 'justified', '2024-01-20', '2024-02-20', 'Materiales urgentes', 'Completamente justificado', admin_profile_id
    ...
    RAISE NOTICE 'Datos de prueba creados exitosamente: % clientes, % ingresos, % gastos, % anticipos', 
        ...
        (SELECT COUNT(*) FROM public.employee_advances WHERE employee_name IN ('Carlos Martínez', 'Ana López'));
