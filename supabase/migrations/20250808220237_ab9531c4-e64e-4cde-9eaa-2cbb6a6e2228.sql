-- Add "general" to department_type enum
ALTER TYPE department_type ADD VALUE 'general';
...
DO $$
BEGIN
  -- Only drop the value if no records reference it
  IF NOT EXISTS (
    SELECT 1 FROM public.profiles WHERE position_enum = 'direccion_general'
  ) THEN
    -- Note: PostgreSQL doesn't support removing enum values directly
    -- We'll need to recreate the enum without direccion_general
