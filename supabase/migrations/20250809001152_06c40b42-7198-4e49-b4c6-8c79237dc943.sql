-- Create enhanced search function that includes both profiles and clients
...
RETURNS TABLE(
  ...
  user_position text, 
  department text,
  user_type text  -- 'employee' or 'client'
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  RETURN QUERY
  -- Get profiles (employees and admins)
  SELECT 
    p.user_id,
    p.id as profile_id,
    p.full_name,
  ...
  -- Get clients
  SELECT 
    NULL::uuid as user_id,  -- Clients don't have auth.users entries
    c.id as profile_id,
    c.full_name,
    c.email,
